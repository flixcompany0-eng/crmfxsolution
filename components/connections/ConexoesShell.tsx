...
import { WindsorAiClient } from "./WindsorAiClient";
import { useT } from "@/hooks/i18n/useT";

interface WindsorContaLinha {
  id: string;
...
export function ConexoesShell({
  ...
  windsorConectado: boolean;
  windsorContas: WindsorContaLinha[];
}) {
  const t = useT();
  ...
  return (
    <Tabs value={aba} onValueChange={(v) => irPara(v, sub)} className="flex flex-col gap-4">
      <TabsList>
        ...
        {/* Isolada de propósito das duas abas acima: aquelas entregam MENSAGEM
            (canal); esta só LÊ métrica de anúncio via Windsor.ai. Ver o
            cabeçalho da migration 0233 para o porquê de não compartilhar
            credencial nem tabela com nenhuma delas. */}
        <TabsTrigger value="windsor">{t("Windsor.ai")}</TabsTrigger>
      </TabsList>
      ...
      <TabsContent value="windsor" className="mt-0">
        <WindsorAiClient conectado={windsorConectado} contas={windsorContas} />
      </TabsContent>
    </Tabs>
  );
}
