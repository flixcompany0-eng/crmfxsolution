  ...
  it("o `@source` cobre toda pasta que realmente escreve className", () => {
    ...
    const raizesComClasse = fs
      ...
      .filter((nome) => {
        ...
        // `branding/` é o mesmo caso: kit de identidade visual entregue como
        // referência (inclui `components/FXBrand.tsx`, um exemplo React
        // isolado), e nenhum arquivo dali é importado pelo app — ver
        // `HANDOFF-fx-solution-branding.md`.
        if (
          ["tests", "docs", "scripts", "supabase", "public", "tasks", "loop", "branding"].includes(
            nome,
          )
        ) {
          return false;
