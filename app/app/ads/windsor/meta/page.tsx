/**
 * Análise → Meta Ads (Windsor.ai). Irmã de `/app/ads/meta` (token direto da
 * Meta), lado a lado e sem se tocarem — a organização pode usar as duas fontes
 * sem uma interferir na outra (ver o cabeçalho da migration 0233).
 *
 * Mesmo piso de papel (`manager`) e mesma regra de segredo fora da tela: esta
 * página só pergunta SE existe conexão, nunca decifra a chave.
 */
...
import { WindsorMetaClient } from "./_components/WindsorMetaClient";

export const metadata = { title: "Meta Ads · Windsor.ai" };
export const dynamic = "force-dynamic";

export default async function WindsorMetaPage() {
  ...
  return (
    <div className="flex h-full flex-col gap-6 overflow-y-auto p-6">
      <header>
        <h1 className="text-2xl font-semibold tracking-tight">{t("Meta Ads (Windsor.ai)")}</h1>
      ...
      {conectado ? (
        <WindsorMetaClient idioma={idioma} />
      ) : (
