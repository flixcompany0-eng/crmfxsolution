...
/**
 * Identidade visual FX Solution: a família de interface trocada de Atkinson
 * Hyperlegible para Outfit (a mesma que a documentação de marca — pedido
 * explícito do kit de identidade FX, `docs/IDENTIDADE-VISUAL.md`: "Use Outfit
 * para interface e IBM Plex Mono para informações técnicas").
 *
 * `docs/white-label.md` documenta a fonte como a ÚNICA coisa de marca que não
 * é configurável pela tela — `next/font` resolve em tempo de BUILD, então
 * trocá-la é necessariamente uma linha de código, não uma linha de banco. É a
 * única exceção deliberada à regra "não mude nenhuma linha de código" deste
 * clone, e está isolada a este arquivo.
 *
 * O identificador da variável CSS (`--font-atkinson`) foi MANTIDO de
 * propósito, mesmo carregando outra fonte agora: `app/globals.css` referencia
 * essa variável em dois lugares (`--font-sans` e a declaração de
 * `font-family`), e `tests/unit/tailwind-tokens.test.ts` vigia esse arquivo.
 * Renomear a variável exigiria tocar `globals.css` — mais uma linha alterada
 * do que o necessário para trocar a fonte. Quem ler `--font-atkinson`
 * carregando Outfit no futuro: é este comentário que explica o descompasso.
 */
const atkinson = Outfit({
