...
export const AUDIT_ACTIONS = [
  ...
  // A conexão da organização com o Windsor.ai (migration 0233) — provedor
  // ISOLADO do `ad_insights_connection` acima: uma é a leitura direta contra a
  // Graph API, esta é a leitura via Windsor.ai (Meta Ads + Instagram na mesma
  // chave). Ação própria pelo mesmo motivo de `ad_insights_connection.*`: o
  // segredo gravado aqui abre dado comercial da organização, e "quem conectou
  // o Windsor.ai desta empresa?" precisa de resposta que não dependa de ler
  // metadata para descobrir qual das duas conexões mudou.
  "windsor_connection.created",
  "windsor_connection.updated",
  "windsor_connection.deleted",
  // Contas de anúncio Meta/Instagram vinculadas sob a conexão Windsor.ai —
  // uma organização pode ter mais de uma (múltiplas contas da mesma empresa).
  "windsor_ad_account.created",
  "windsor_ad_account.deleted",
  // Sincronização manual disparada pela tela (não pelo cron): registra quem
  // pediu a atualização e quando, útil quando o dado parece desatualizado.
  "windsor_sync.completed",
] as const;
